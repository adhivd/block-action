...
contract BlockAction {
    ...
    /*
        BlockAction.at('0x4acecbe230cf637ac5f07934e2b57916bb72bc36').start_suit(["0x14723a09acff6d2a60dcdf7aa4aff308fddc160c", "0x4b0897b0513fdc7c541b6d9d7e929c4e5364d2db", "0x583031d1113ad414f02576bd6afabfb302140225"], [333333,333333,333333])
    ...
    */
    ...
    function start_suit(address[] class_members, uint[] shares) public returns (uint error_code){
        require (msg.sender == admin);
        require (class_members.length == shares.length);
        for (uint i = 0; i < class_members.length; i++) {
            members[class_members[i]].share = shares[i];
        }
    ...
    function transfer(address from, address to, uint shares_to_transfer) public returns (uint error_code) {
        if (msg.sender != admin || msg.sender != from) return 1; //incorrect sender
